...
test("Phase 7 loop: buying pet care leaves skincare next for c2", () => {
  // The spec's acceptance case: pet care → purchase → prompted toward skincare.
  const before = rankEligible(getEligible(byId("c2")));
  assert.deepEqual(slugs(before), ["pet-care", "skincare", "stationery"]);
  ...
  assert.equal(after[0].category, "skincare", "the forward prompt has a target");
});
