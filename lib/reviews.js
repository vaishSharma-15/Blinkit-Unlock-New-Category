...
export function getProductReviews(product, limit = 10) {
  ...
  const relevant = all.filter(
    (r) => mentionsBrand(r.text, product.id) && !mentionsConflictingGroup(r.text, own),
  );
...
export function unattributedReviews(categorySlug) {
  ...
  // Same two-part test as getProductReviews: a review only counts as
  // "attributed" if it clears both checks for some product in the category.
  // Using brand-match alone here would make the Drools/cat-food review
  // count as "attributed" to the Drools dog-food product, when it's
  // actually excluded from grounding anything in this catalogue — it would
