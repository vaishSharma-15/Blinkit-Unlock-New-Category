...
test("a review must name the product's own brand to ground it — no loose category fallback", () => {
  // Nivea Soft Light Moisturiser. The skincare corpus has 18 reviews; only
  // one names "Nivea". Every other skincare review — about Plum, Streax,
  // Garnier, or nothing specific — must be excluded, not treated as
  // close-enough colour for this product.
  const reviews = getProductReviews(product("skn-004"), 20);
  assert.equal(reviews.length, 1);
  assert.match(reviews[0].text, /Nivea/);
});
